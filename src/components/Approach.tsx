...
function Abordagem() {
  return (
    <section id="abordagem" className="bg-navy text-warm-white py-12 sm:py-16 lg:py-24 px-6 sm:px-10 lg:px-16 grid grid-cols-1 lg:grid-cols-[1fr_1.5fr] gap-8 lg:gap-20 items-center">
      <div>
        <p className="before:content-[''] before:block before:w-6 before:h-px before:bg-teal-light flex items-center text-[.75rem] tracking-[.2em] uppercase text-teal-light mb-4 gap-3">Minha abordagem</p>
        <h2 className="font-serif text-[clamp(2rem,3vw,2.8rem)] font-light leading-[1.2] text-warm-white">
          Medicina que <em className="italic text-teal-light">ouve</em> antes de prescrever
        </h2>
      </div>
      <div className="flex flex-col gap-8">
        {passos.map((passo) => (
          <div key={passo.num} className="grid grid-cols-[48px_1fr] gap-5 items-start">
            <span className="font-serif text-[2rem] font-light text-teal-light leading-none pt-0.5">{passo.num}</span>
            <div>
